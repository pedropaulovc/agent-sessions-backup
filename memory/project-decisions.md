...
- Public GitHub repo; protected `main`-only deploy. ~~Workers Builds auto-deploy + DEV_AUTH bearer previews~~ SUPERSEDED 2026-08: per-PR previews are provisioned by the `preview` job in CI, from the PR's own run, in a dedicated non-production account (PRs #83–#116, collapsed back into CI by #139; `infra/cf/deploy.md`), and DEV_AUTH is fully retired (PR #118).
- **Auth model (2026-08-13, PRs #117–#119): "machine certs ingest, passkeys egress."** Machine mTLS certs are write/ingest credentials plus content-free aggregates; on the machine API, session content reads (`/sessions*`, `/search`) require a passkey-minted short-lived read grant (`agent-sessions auth`) — the viewer's own passkey session remains the separate human egress path. Uploads are append-guarded: a non-append rewrite preserves the displaced object under write-once `raw-prev/` keys (90-day R2 lifecycle rule) so a stolen write-only cert can displace but never destroy backups.
