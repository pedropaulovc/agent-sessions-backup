...
- **Auth model (2026-08-13, PRs #117–#119): "machine certs ingest, passkeys egress."** Machine mTLS certs are write/ingest credentials plus content-free aggregates; session content reads (`/sessions*`, `/search`) require a passkey-minted short-lived read grant (`agent-sessions auth`). Uploads are append-guarded: a non-append rewrite preserves the displaced object under write-once `raw-prev/` keys (90-day R2 lifecycle rule) so a stolen write-only cert can displace but never destroy backups.
