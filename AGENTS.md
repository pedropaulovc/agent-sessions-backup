...
- Open PR `<number>` with `npm --prefix hub run preview:open -- --pr <number>`. It opens the PR's self-contained preview at `https://pr-<number>-app.sessions-ppe.workers.dev/?token=…`, deriving the per-PR bearer from `~/.config/agent-sessions/preview-seed` (or `PREVIEW_BEARER_SEED`); visiting once sets the preview session cookie. Use `--print-only` when a non-interactive job needs the URL without launching a browser.
- The same-repository `preview` job posts the derived per-PR bearer as a public login code in a PR comment; it never posts `PREVIEW_BEARER_SEED`. This is safe only because each preview is isolated to the disposable non-production account and contains no production data.
- Production-session debugging is a hand-carried zip: the owner clicks "Export zip" on the session page of the production viewer (behind their passkey session), sanitizes the archive, and hands it over; the agent pushes it into a preview with `node hub/scripts/preview-upload-session.mjs --pr <number> --zip <file>`. The derived bearer is public in PR comments, so preview uploads must contain no sensitive data.
...
- A PR's `preview` CI job holds the non-production API token (`CLOUDFLARE_PPE_API_TOKEN`) and the bearer seed, and deploys that PR's own environment from that PR's own checkout — a PR environment is meant to be safe for anyone to deploy, so there is no release handoff to a privileged workflow. The blast radius is the throwaway `cbb04a26…` account and nothing else. Production deployment remains protected and `main`-only, with its own separate tokens.
