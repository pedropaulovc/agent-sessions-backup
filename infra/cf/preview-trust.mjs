...
/**
 * The per-PR preview bearer is derived from one standing seed. Every principal that needs it
 * (the preview job baking it into the Worker, the smoke/e2e steps, the owner's machines via
 * `~/.config/agent-sessions/preview-seed`) derives the same token. The standing seed never
 * enters public logs, job outputs, or PR comments; the derived token is intentionally published
 * as that disposable preview's public login code. The Worker holds only the derived per-PR token:
 * reading one preview's Worker vars exposes that one disposable preview, not the seed.
 */
