...
| `curl: (58)` "could not load PEM client certificate" / "Private key does not match the certificate public key" | **Your client material**, before any network traffic. Measured 2026-09-02: a garbage `--cert` file and a real cert with the wrong `--key` both exit 58, not 35. | Fix the PEM/key pair you passed. Nothing about the edge is being tested yet. |
| `curl: (35)` TLS handshake failure, or `curl: (60)` server-certificate verification failure | **Edge TLS layer — but neither exit code says *which* problem.** Do not read either as "no edge certificate yet" without the evidence in the next column. Measured 2026-09-02 on this zone: a plain `curl` with **no** `--cert/--key` completes the handshake and gets an HTTP `403` from the §2 WAF rule (`-v` shows `TLS handshake, Request CERT (13)`, i.e. the edge asked for a client cert); and an unrouted 3-label host under `sessions.pedrovc.com.br` (`--resolve` past DNS) is served the viewer's `*.sessions.pedrovc.com.br` cert and returns HTTP `530`, not exit 60 — so on this zone "no edge cert yet" mostly presents as the `530` row below. The live `api.sessions.pedrovc.com.br` cert is its own (SAN `api.sessions.pedrovc.com.br, *.api.sessions.pedrovc.com.br`, issuer Google Trust Services WE1), consistent with `infra/cf/mtls.md:24-26`. | Get evidence, then pick: (1) re-run with no `--cert/--key` — any HTTP status means the edge and its cert are fine and the failure is in what you passed; (2) `echo \| openssl s_client -connect HOST:443 -servername HOST 2>/dev/null \| openssl x509 -noout -ext subjectAltName` — SAN does **not** cover the host → edge cert not provisioned yet, wait for the Workers custom-domain cert; SAN covers it but `curl -v` says `unable to get local issuer certificate` → your local CA bundle, not Cloudflare. Not a CA-association problem in any of these cases. |
...
you got an HTTP status at all. No HTTP status = you never reached L7: exit 6 is DNS, exit 58 is your
own client material, exit 35/60 is edge TLS and needs the SAN check above to say *why*. An HTTP
401/403 = the request reached Cloudflare's L7 and the client-cert step is what failed.
