 ...
 */
import type { CostTotals } from '../session-cost';

/** Whether a session (or subtree) has a cost to show at all, and if so what.
 *
 * Three states rather than one string with two magic values, because "no usage records" and
 * "records we could not price" are different claims and the wording for them differs per surface:
 * a table cell wants `—`, a chip in a row of chips wants `cost unknown`, a labelled line wants
 * `unknown`. The DECISION is the part that must not be re-derived, so it lives here and the
 * phrasing stays at the call site. */
export type CostLabel = { state: 'none' } | { state: 'unknown' } | { state: 'known'; text: string };

export function costLabel(cost: CostTotals): CostLabel {
  if (cost.calls === 0) return { state: 'none' };
  if (cost.pricedCalls === 0 || cost.usd === null) return { state: 'unknown' };
  return { state: 'known', text: knownCost(cost.usd, cost.pricedCalls, cost.calls) };
}
