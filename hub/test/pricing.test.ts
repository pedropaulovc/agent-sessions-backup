...
describe('cache accounting', () => {
  // Accounting is attached to the transcript row, not the catalog rate. The same counters must
  // therefore be evaluated with an explicit source basis.
  const tokens = { input_tokens: 100_000, output_tokens: 10_000, cache_read_tokens: 90_000 };

  it('charges transcript-source disjoint cache reads ON TOP of input', () => {
    const c = costOfUsage({ model: 'claude-opus-5', ...tokens, cache_basis: 'disjoint' }, OPUS);
  ...
  it('subtracts transcript-source subset cache reads FROM input', () => {
    const c = costOfUsage({ model: 'gpt-5.6-luna', ...tokens, cache_basis: 'subset' }, LUNA);
  ...
  it('pins the real high-cache ratio to the row basis', () => {
    // These counters are from the observed OMP/OpenAI regression. With the stated $10/$1/$50
    // rates, disjoint arithmetic bills 21.4M fresh + 814.2M cached + 1.8M output = $1,118.20.
    // Subset arithmetic clamps the cache charge to the 21.4M input prefix, yielding $111.40.
    ...
    const counters = { model: 'regression-model', input_tokens: 21_400_000, cache_read_tokens: 814_200_000, output_tokens: 1_800_000 };
    const disjoint = costOfUsage({ ...counters, cache_basis: 'disjoint' }, price);
    const subset = costOfUsage({ ...counters, cache_basis: 'subset' }, price);
    expect(disjoint.usd).toBeCloseTo(1_118.2, 8);
    expect(subset.usd).toBeCloseTo(111.4, 8);
    expect(disjoint.byClass.input).toBeCloseTo(214, 8);
    expect(disjoint.byClass.cacheRead).toBeCloseTo(814.2, 8);
  });
  ...
  it('never bills negative fresh input when cache_read exceeds input', () => {
    ...
    const c = costOfUsage(
      { model: 'gpt-5.6-luna', input_tokens: 500, output_tokens: 0, cache_read_tokens: 9_000, cache_basis: 'subset' },
      LUNA,
    );
  ...
  it('takes the per-row clamp sum for a group rather than clamping the group totals', () => {
    ...
    const group = {
      ...
      cache_basis: 'subset' as const,
    };
    expect(costOfUsage(group, LUNA).billableInputTokens).toBe(10_000);
  });

  it('ignores fresh_input_tokens for disjoint rows, where input is already fresh', () => {
    const c = costOfUsage(
      { model: 'claude-opus-5', input_tokens: 10_500, cache_read_tokens: 9_000, fresh_input_tokens: 10_000, cache_basis: 'disjoint' },
      OPUS,
    );
    expect(c.billableInputTokens).toBe(10_500);
  });

  it('fails closed when the transcript basis is NULL', () => {
    // A missing source classification is not evidence of either equation. Even a zero-token row
    // must remain visibly unpriced rather than turning an unknown basis into a free call.
    const c = costOfUsage({ model: 'claude-opus-5', input_tokens: 0, cache_basis: null }, OPUS);
...
describe('batch tier', () => {
  it('halves input and output when the provider publishes a batch rate', () => {
    const u = { model: 'gpt-5.6-luna', input_tokens: 1_000_000, output_tokens: 1_000_000, cache_basis: 'subset' as const };
  ...
  it('applies a published batch rate per class, not all-or-nothing', () => {
    // A provider that publishes an input rate but no output one is common enough (the fields are
    // independent upstream). The old check required BOTH before using EITHER, so an input-only
    // row silently paid full list price despite a published batch rate for exactly the tokens it
    // had.
    const halfBatch: ModelPrice = { ...LUNA, output_cost_batch: null };
    const inputOnly = { model: 'gpt-5.6-luna', input_tokens: 1_000_000, output_tokens: 0, cache_basis: 'subset' as const };
    ...
    const both = { model: 'gpt-5.6-luna', input_tokens: 1_000_000, output_tokens: 1_000_000, cache_basis: 'subset' as const };
  ...
  it('falls back to standard rates when the provider has no batch tier', () => {
    ...
    const u = { model: 'claude-opus-5', input_tokens: 1_000_000, output_tokens: 0, cache_basis: 'disjoint' as const };
...
describe('missing prices', () => {
  it('flags unpriced instead of charging zero', () => {
    const c = costOfUsage({ model: 'brand-new-model', input_tokens: 1e6, output_tokens: 1e6, cache_basis: 'disjoint' }, null);
    expect(c.unpriced).toBe(true);
    expect(c.usd).toBe(0);
  });
  ...
  it('treats a missing cache READ rate as the input rate, not as free', () => {
    ...
    const c = costOfUsage({ model: 'claude-opus-5', cache_read_tokens: 1_000_000, cache_basis: 'disjoint' }, noReadRate);
  ...
  it('flags a row as unpriced when its 5m cache-write rate is unpublished', () => {
    ...
    const c = costOfUsage({ model: 'claude-opus-5', cache_creation_5m_tokens: 1_000_000, cache_basis: 'disjoint' }, noW5);
    expect(c.unpriced).toBe(true);
    expect(c.usd).toBe(0);
  });
  ...
  it('flags a row as unpriced when its 1h cache-write rate is unpublished', () => {
    ...
    const c = costOfUsage({ model: 'claude-opus-5', cache_creation_1h_tokens: 1_000_000, cache_basis: 'disjoint' }, noW1h);
    expect(c.unpriced).toBe(true);
    expect(c.usd).toBe(0);
  });
  ...
  it('still prices a row whose missing write rate is never multiplied by anything', () => {
    ...
    const c = costOfUsage({ model: 'claude-opus-5', input_tokens: 1_000_000, cache_basis: 'disjoint' }, noWrites);
  ...
  it('reports no billable input for a row it cannot price', () => {
    ...
    const c = costOfUsage({ model: 'brand-new-model', input_tokens: 50_000, cache_read_tokens: 40_000, cache_basis: 'disjoint' }, null);
    expect(c.unpriced).toBe(true);
    expect(c.billableInputTokens).toBe(0);
  ...
  it('requires a rate only for token classes that are actually present', () => {
    ...
    const inputOnly = costOfUsage({ model: 'claude-opus-5', input_tokens: 1_000_000, output_tokens: 0, cache_basis: 'disjoint' }, noOutputRate);
    ...
    const withOutput = costOfUsage({ model: 'claude-opus-5', input_tokens: 1_000_000, output_tokens: 1, cache_basis: 'disjoint' }, noOutputRate);
    expect(withOutput.unpriced).toBe(true);

    const noInputRate: ModelPrice = { ...OPUS, input_cost: null };
    const outputOnly = costOfUsage({ model: 'claude-opus-5', input_tokens: 0, output_tokens: 1_000_000, cache_basis: 'disjoint' }, noInputRate);
...
describe('corrupt token counters', () => {
  it('never produces a negative cost from a negative counter', () => {
    ...
    const c = costOfUsage({ model: 'claude-opus-5', input_tokens: -1_000_000, output_tokens: 40, cache_basis: 'disjoint' }, OPUS);
    expect(c.usd).toBeGreaterThanOrEqual(0);
...
describe('rate set reporting', () => {
  it('reports which rate set actually priced the row', () => {
    const u = { model: 'gpt-5.6-luna', input_tokens: 1_000_000, output_tokens: 0, cache_basis: 'subset' as const };
    ...
    expect(costOfUsage({ model: 'claude-opus-5', input_tokens: 1, cache_basis: 'disjoint' }, OPUS, { batch: true }).rateSet).toBe('standard');
  });
  ...
  it('falls a missing cache-read rate back to the STANDARD input rate, not the batch one', () => {
    ...
    const u = { model: 'gpt-5.6-luna', input_tokens: 1_000_000, cache_read_tokens: 1_000_000, cache_basis: 'subset' as const };
  ...
  it('does not report a zero-token call as unpriced when its basis is known', () => {
    // A known basis still lets a zero-token row remain a genuine $0 even without a matching price.
    // The separate NULL-basis case above is deliberately different: it is unknown data, not free.
    const c = costOfUsage({ model: 'brand-new-model', input_tokens: 0, output_tokens: 0, cache_basis: 'disjoint' }, null);
    expect(c.unpriced).toBe(false);
    expect(c.usd).toBe(0);
    expect(c.rateSet).toBe('none');
    // A row with tokens and no price is still unpriced — the signal has to keep working.
    expect(costOfUsage({ model: 'brand-new-model', input_tokens: 1, cache_basis: 'disjoint' }, null).unpriced).toBe(true);
  });
  ...
  it('claims no rate set at all when every billable class is zero', () => {
    ...
    const empty = { model: 'gpt-5.6-luna', input_tokens: 0, output_tokens: 0, cache_basis: 'subset' as const };
  ...
  it('counts cache classes as standard — LiteLLM publishes no batch cache rates', () => {
    ...
    const written = { model: 'gpt-5.6-luna', input_tokens: 1_000_000, cache_creation_5m_tokens: 1_000, cache_basis: 'subset' as const };
    expect(costOfUsage(written, LUNA, { batch: true }).rateSet).toBe('mixed');
    // And with no cache tokens at all it stays a clean 'batch'.
    const plain = { model: 'gpt-5.6-luna', input_tokens: 1_000_000, output_tokens: 1_000_000, cache_basis: 'subset' as const };
...
describe('rate signature basis', () => {
  it('keeps disjoint and subset rows distinct even with identical rates', () => {
    const counters = { model: 'same-model', input_tokens: 100, cache_read_tokens: 90 };
    const disjoint = costOfUsage({ ...counters, cache_basis: 'disjoint' }, OPUS);
    const subset = costOfUsage({ ...counters, cache_basis: 'subset' }, OPUS);
    expect(disjoint.rateSignature).not.toBe(subset.rateSignature);
  });
});
...
describe('per-class breakdown', () => {
  ...
  const CASES: Array<[string, Parameters<typeof costOfUsage>[0], ModelPrice, boolean]> = [
    ['disjoint, every class', { model: 'claude-opus-5', input_tokens: 100_000, output_tokens: 10_000,
      cache_read_tokens: 90_000, cache_creation_5m_tokens: 5_000, cache_creation_1h_tokens: 2_000,
      cache_basis: 'disjoint' }, OPUS, false],
    ['subset, cache exceeds input', { model: 'gpt-5.6-luna', input_tokens: 500, cache_read_tokens: 9_000,
      cache_basis: 'subset' }, LUNA, false],
    ['batch tier', { model: 'gpt-5.6-luna', input_tokens: 1_000_000, output_tokens: 1_000_000,
      cache_basis: 'subset' }, LUNA, true],
    ['output only', { model: 'claude-opus-5', output_tokens: 1_000_000, cache_basis: 'disjoint' }, OPUS, false],
  ];
  ...
  it('attributes the cache-read charge at the READ rate, not the input rate', () => {
    ...
    const c = costOfUsage(
      { model: 'gpt-5.6-luna', input_tokens: 500, cache_read_tokens: 9_000, cache_basis: 'subset' },
      LUNA,
      { batch: true },
    );
  ...
  it('gives every unpriced result its OWN zero breakdown', () => {
    ...
    const a = costOfUsage({ model: 'nope', input_tokens: 1_000_000, cache_basis: 'disjoint' }, null);
    const b = costOfUsage({ model: 'nope', input_tokens: 1_000_000, cache_basis: 'disjoint' }, null);
    expect(a.byClass).not.toBe(b.byClass);
    a.byClass.input += 999;
    expect(b.byClass.input, 'the zero breakdown is shared between calls').toBe(0);
    expect(costOfUsage({ model: 'nope', cache_basis: 'disjoint' }, null).byClass.input).toBe(0);
  });

  it('reports every class as zero on an unpriced row', () => {
    const c = costOfUsage({ model: 'nope', input_tokens: 1_000_000, cache_basis: 'disjoint' }, null);
    expect(c.unpriced).toBe(true);
...
describe('rates that overflow when costed', () => {
  it('reports a row whose cost overflows to Infinity as unpriced, not as priced-with-null', () => {
    ...
    const c = costOfUsage({ model: 'gpt-5.6-luna', input_tokens: 1000, cache_basis: 'subset' }, huge);
  ...
  it('leaves a large but representable cost alone', () => {
    ...
    const c = costOfUsage({ model: 'gpt-5.6-luna', input_tokens: 1000, cache_basis: 'subset' }, big);
    expect(c.unpriced).toBe(false);
...
describe('subset cache-read clamp', () => {
  it('never bills more cached tokens than the row reports as input', () => {
    ...
    // says it used.
    const c = costOfUsage(
      { model: 'gpt-5.6-luna', input_tokens: 500, cache_read_tokens: 9000, cache_basis: 'subset' },
      LUNA,
    );
  ...
  it('leaves disjoint models alone', () => {
    ...
    const c = costOfUsage(
      { model: 'claude-opus-5', input_tokens: 500, cache_read_tokens: 9000, cache_basis: 'disjoint' },
      OPUS,
    );
  ...
  it('treats a fully clamped cache read as costing nothing, not as needing a rate', () => {
    ...
    const c = costOfUsage(
      { model: 'gpt-5.6-luna', input_tokens: 0, cache_read_tokens: 5000, cache_basis: 'subset' },
      noReadRate,
    );
  ...
  it('uses the per-row sum when the caller pre-aggregated', () => {
    ...
    const c = costOfUsage(
      { model: 'gpt-5.6-luna', input_tokens: 1000, cache_read_tokens: 18000, billable_cache_read_tokens: 1000, cache_basis: 'subset' },
      LUNA,
    );
