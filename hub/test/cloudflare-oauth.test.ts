...
async function authorize(overrides: Record<string, unknown> = {}): Promise<void> {
 ...
 const callback = new URL(`${VIEWER}/oauth/cloudflare/callback`);
...
describe('Cloudflare OAuth broker', () => {
 it('creates a five-minute Authorization Code + PKCE request with the exact managed-CA scope', async () => {
  ...
  expect(url.searchParams.get('redirect_uri')).toBe(`${VIEWER}/oauth/cloudflare/callback`);
 ...
 it('preserves a pending authorization when a public callback has the wrong state', async () => {
  const authorizeUrl = await authorizationUrl();
  const wrongState = new URL(`${VIEWER}/oauth/cloudflare/callback`);
 ...
 it('proxies only the configured zone and never returns its bearer credential', async () => {
  ...
  const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
   ...
   // The assertion is that the broker pins the CONFIGURED zone, so it reads the same
   // var the Worker does; a literal zone id would only restate wrangler.jsonc and go
   // stale the moment the zone moves.
   expect(request.url).toBe(
    `https://api.cloudflare.com/client/v4/zones/${testEnv.CF_ZONE_ID}/client_certificates/${certId}`,
   );
...
describe('Cloudflare OAuth routes', () => {
 it('allows only a current admin certificate to begin authorization', async () => {
  ...
  const current = new Request(`${API}/api/v1/admin/cloudflare-oauth/start`, {
   method: 'POST',
  ...
  const anonymous = new Request(`${API}/api/v1/admin/cloudflare-oauth/start`, { method: 'POST' });
 ...
 it('accepts the browser callback only on the viewer hostname', async () => {
  const viewer = new Request(`${VIEWER}/oauth/cloudflare/callback?code=x&state=wrong`);
  expect((await route(viewer, testEnv, {} as ExecutionContext)).status).toBe(400);
  const api = new Request(`${API}/oauth/cloudflare/callback?code=x&state=wrong`);
