...
function baseManifestAtRef(ref, candidate) {
  let manifestResult = git(['show', `${ref}:hub/migrations/manifest.json`]);
  if (manifestResult.status === 0) return validateManifestShape(JSON.parse(manifestResult.stdout.toString('utf8')));
  ...
  if (tree.status !== 0) {
    ...
    manifestResult = git(['show', `${ref}:hub/migrations/manifest.json`]);
    if (manifestResult.status === 0) return validateManifestShape(JSON.parse(manifestResult.stdout.toString('utf8')));
...
async function main() {
  ...
  if (command === 'check-base') {
    const result = await loadAndValidateManifest({ migrationsDir, manifestPath });
